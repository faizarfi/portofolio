  ...
  return (
    <>
      ...
      <button
        ...
        className="group relative flex items-center gap-1.5 rounded-lg border border-slate-300 bg-white/90 px-2.5 py-1.5 font-mono text-xs font-semibold text-slate-800 shadow-2xs transition-all hover:border-slate-400 hover:bg-slate-50 hover:text-slate-950 focus-visible:outline-2"
      >
        <span className="text-sm leading-none">{activeLangObj.flag}</span>
        <span className="uppercase text-[11px] font-bold tracking-wider text-slate-700 group-hover:text-slate-950">
          {activeLangObj.code.split("-")[0]}
        </span>
        <FontAwesomeIcon
          icon={faChevronDown}
          className="h-2.5 w-2.5 text-slate-400 transition-transform group-hover:text-slate-700"
        />
