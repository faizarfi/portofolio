...
export default function ProjectCard({ project }: { project: Project }) {
  return (
    <div className="neat-card group flex h-full flex-col overflow-hidden">
      {/* ── Screenshot / Preview ── */}
      <Link
        href={`/projects/${project.slug}`}
        aria-label={`Lihat detail proyek ${project.name}`}
        className="relative block aspect-16/10 w-full overflow-hidden bg-slate-100 border-b border-slate-100"
      >
        {project.image ? (
          <Image
            src={project.image}
        ...
        <div className="mt-auto flex items-center gap-2 border-t border-slate-100 pt-4">
          ...
          {project.url && project.url !== "/" && (
            <a
              ...
              className="inline-flex flex-1 items-center justify-center gap-1.5 rounded-xl bg-blue-600 px-3 py-2 text-xs font-bold text-white shadow-xs transition-all hover:bg-blue-500"
            >
