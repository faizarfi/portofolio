  ...
  if (!mounted) {
    return (
      <button
        ...
        className="flex h-8 w-8 sm:h-9 sm:w-9 items-center justify-center rounded-lg border border-slate-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 text-slate-700 dark:text-zinc-300 shadow-2xs shrink-0"
      >
  ...
  return (
    <div className="relative shrink-0">
      ...
      <button
        ...
        className="group flex h-8 w-8 sm:h-8.5 sm:w-8.5 items-center justify-center rounded-full border border-slate-200/80 dark:border-zinc-700/80 bg-slate-100/80 dark:bg-zinc-850 text-slate-700 dark:text-zinc-200 shadow-2xs transition-all hover:border-slate-400 dark:hover:border-zinc-600 hover:bg-white dark:hover:bg-zinc-800 hover:text-slate-950 dark:hover:text-white shrink-0"
      >
        {resolvedTheme === "dark" ? (
      ...
      {isOpen && (
        <>
          ...
          <div className="absolute right-0 top-full mt-2 z-50 w-48 overflow-hidden rounded-xl border border-slate-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-1.5 shadow-xl font-mono text-xs animate-fade-in">
            <button
              onClick={() => changeTheme("system")}
              className={`flex w-full items-center justify-between rounded-lg px-3 py-2 text-left transition-colors ${
                mode === "system"
                  ? "bg-slate-100 dark:bg-zinc-800 text-slate-950 dark:text-white font-bold"
                  : "text-slate-700 dark:text-zinc-300 hover:bg-slate-50 dark:hover:bg-zinc-800/60"
              }`}
            >
              <div className="flex items-center gap-2">
                <FontAwesomeIcon icon={faLaptop} className="h-3.5 w-3.5 text-slate-500 dark:text-zinc-400" />
